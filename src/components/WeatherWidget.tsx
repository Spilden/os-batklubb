  ...
  return (
    ...
    >
      <div className="flex flex-col items-center gap-2">
        ...
        <CurrentIcon className={variant === 'compact' ? 'w-8 h-8' : 'w-12 h-12'} />
        <p className={variant === 'compact' ? 'text-xl font-bold' : 'text-3xl font-bold'}>
          {Math.round(current.temperature)}°C
        </p>
        {variant === 'full' && <p className="text-sm text-text-muted">{currentInfo.label}</p>}
        {variant === 'full' && (
        ...
        {forecast.map((day) => {
          ...
          return (
            <div key={day.time} className="flex flex-col items-center gap-1">
              ...
              {variant === 'full' && <p className="text-xs text-text-muted">{label}</p>}
              {variant === 'full' && (
