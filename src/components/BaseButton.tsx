...
export default function BaseButton({ children, onClick, variant = 'primary', className }: BaseButtonProps) {
  const variants = {
  ...
  return (
    <button
    onClick={onClick}
    className={`${variants[variant]} ${className}  px-6 py-3 rounded-lg text-sm uppercase transition-colors`}
    >
      {children}
    </button>
  )
